...
# Copy the server configuration directly into the container
COPY --chown=1001:0 server.xml /config/

# Pre-compile the server features defined in server.xml
RUN features.sh

# Warm up the OpenJ9 class cache for faster container startup
RUN configure.sh